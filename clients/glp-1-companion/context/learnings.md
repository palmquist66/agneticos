...
- 2026-05-26: When asked "what's next," verify actual ground truth (git status, what's committed, what's deployed) before trusting a planning/status tracker. The implementation-plan said Phase 9 code was "done," but `git status` revealed the entire mobile-facing REST API (today/trends/log/meds/user/onboarding/health-sync) was uncommitted and never pushed — so production didn't have it, and the runbook's "install native deps first" order was wrong. The real blocker was deploy, not native build. Tracker checkboxes describe intent; git/deploy state describes reality.
- 2026-05-26: Determine the git root with `git rev-parse --show-toplevel` before reasoning about what's tracked. Ran `git ls-files` from a parent dir (`AgneticOS/`) and wrongly concluded the Expo `mobile/` app wasn't version-controlled; the real root was `AgneticOS/agentic-os/` and it tracks `mobile/` fine. Only the `app/` backend is a separate nested repo (its own remote, deploys to Vercel).
